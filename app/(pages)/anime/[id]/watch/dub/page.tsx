...
export default function SubPage() {
  ...
  useEffect(() => {
    if (state.dubStream?.data) {
      const isAvailable = !!state.dubStream?.data?.stream_links.find(
        (s) => s.name === "mp4upload"
      );
      const isStrAv = isAvailable ? state.dubStream?.data : false;
    ...
    <div className="p-2">
      ...
      {currentlyPlayed &&
        (currentlyPlayed === "live" ? (
          <VideoPlayer
            src={`https://api-anikii.onrender.com/anime/${id}/stream/ep/1/live?type=dub`}
      ...
      <div className="w-full bg-secondary/20 rounded-b-md">
        ...
        <div className="w-full overflow-x-auto flex snap-mandatory snap-x *:snap-center gap-2 p-2 scroll-px-2">
        {
            ...
            className={
              clsx(
                "w-fit cursor-pointer bg-tertiary py-2 px-4 flex gap-2 text-accent items-center justify-center rounded-md",
                currentlyPlayed === "live"&& "bg-tertiary/50"
              )
            }
          ...
          {state.dubStream?.data?.stream_links.map((i, ind) => {
            return (
              <div
                className={
                  clsx(
                    "w-fit cursor-pointer bg-tertiary py-2 px-4 flex gap-2 text-accent items-center justify-center rounded-md",
                    currentlyPlayed!=="live"&& currentlyPlayed?.url=== i.url && "bg-tertiary/50"
                  )
                }
                key={ind}
